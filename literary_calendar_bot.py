        ...
        while i < len(lines):
            ...
            if line.startswith('# '):
                ...
                for j in range(i + 1, min(i + 15, len(lines))):
                    ...
                    # Ищем ссылки на svetapp.rusneb.ru
                    if 'svetapp.rusneb.ru' in link_line:
    ...
    def format_event_message(self, event: Dict, books: List[Dict] = None) -> str:
        ...
        if books:
            ...
            for book in books[:5]:  # Максимум 5 книг
                ...
                if book_slug:
                    book_url = f"https://svetapp.rusneb.ru/catalog/{book_slug}"
            ...
            for db_event in db_events:
                ...
                if db_event['reference_uuid']:
                    if db_event['event_type'] == 'author':
                        ...
                        event['links'].append(
                            f"https://svetapp.rusneb.ru/catalog?authors={db_event['reference_uuid']}&page=1"
                        )
                    elif db_event['event_type'] == 'tag':
                        event['tags'].append(db_event['reference_uuid'])
                        event['links'].append(
                            f"https://svetapp.rusneb.ru/catalog?tags={db_event['reference_uuid']}&page=1"
                        )
                    elif db_event['event_type'] == 'category':
                        event['links'].append(
                            f"https://svetapp.rusneb.ru/catalog?categories={db_event['reference_uuid']}&page=1"
                        )
